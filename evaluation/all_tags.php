...
<html lang="en">
  <head>
    ...
    <title>Manzanilla · Evaluation > Image tags</title>
      ...
      $(function (){
        ...
        mnz.aunthenticate(function(err, response){
            Manzanilla.loadImageMedium($('#id-image').val(),function(){
              ...
              vpks = new VPKS(image_path, 'the-canvas', 'canvas-container', mnz.username,true, false, true);
              ...
              Manzanilla.getAllImagesAnnotationsByUser($("#image").val(), function(results){
                ...
                $.each(results, function(username, annotations){
                  ...
                  if (typeof(annotations.category) !== 'undefined'){
                    $('#category-list').append('<div class="clear"><strong>' + username + ':</strong></div>');
                  ...
                  if (typeof(annotations.concepts) !== 'undefined'){
                    $('#concepts-list').append('<div class="clear"><strong>' + username + ':</strong></div>');
                    $('#concepts-list').append('<div class="relation"> ' +  annotations.concepts.map(function(concept){ return concept.data.concept}).join('</div><div class="relation">') + '</div>');
                    $('#concepts-list').append('<br/>');
                  }

                  if (typeof(annotations.relations) !== 'undefined'){

                    $('#relation-list').append('<div class="clear"><strong>' + username + ':</strong></div>');
                    $('#relation-list').append(annotations.relations.map(function(category){

                        console.log('source',category.data.source); 
                    ...
                    $('#relation-list').append('<br/>');
                  }

                  if (typeof(annotations.vpks) !== 'undefined'){

                    annotations.vpks.map(function(vpk){ 
